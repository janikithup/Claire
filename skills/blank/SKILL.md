...
3. Leak-check the brief, then show it: dispatch `brief-leak-auditor` on the de-primed brief first (it reads only the brief and judges whether it leaks your lean); fix any lean it names; then print the `[DEPRIMED-BRIEF]` block, per `/claire:challenge` Step 3. Do NOT self-certify the brief as neutral — that is the failure this step exists to stop.
4. Dispatch `blank-slate-advisor` with the leak-checked neutral brief, `[DEPRIMED-BRIEF]` on its own line before the situation — no files, no rationale, no history.
