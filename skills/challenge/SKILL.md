...
1. **Leak-check (mandatory, before any dispatch).** Build the brief **once** as a single string whose first line is literally `[DEPRIMED-BRIEF]`, followed by the brief body — this exact string is what *both* the auditor and the critic receive. Dispatch `brief-leak-auditor` with **that string, verbatim** (its persona is in the agent file; it reads the body after the tag and judges whether it betrays your lean). The receipt fingerprints the text **after** the tag, so a wrapper before the tag is harmless — but **do not edit the string once it passes, and never append anything after the body.** The gate matches the critic's brief against the receipt by **exact equality** (0.6.2): a single character changed between audit and dispatch, or any text added after the body, breaks the match and is correctly flagged — because that changed text was never audited. This is the exact failure that let a steer through once (a clean audit on the situation+question, then a steering "ask" appended before dispatch). Do NOT skip this on the grounds that "my brief is already neutral" or "the adversary will catch any lean anyway" — that is the exact rationalisation that defeats the gate; the producer's self-assessment is the unreliable thing this step exists to replace. (Mechanism, v0.2.0+: a clean verdict writes a short-lived *receipt* — a fingerprint of the passed brief. The gate checks for that receipt, **not** the tag. Typing `[DEPRIMED-BRIEF]` without an actual clean audit no longer silences the gate — so there is no shortcut around this step.)
...
3. **Dispatch** the chosen primitive **by its namespaced name** — `claire:failure-mode-attacker`, `claire:blank-slate-advisor`, etc. — never the bare name. A workspace may have its own same-named local agent that would otherwise shadow Claire's and silently run instead; the `claire:` prefix guarantees you get Claire's version, and it is also what lets the de-priming gate recognise the dispatch. Structure the Agent prompt so that **everything after the `[DEPRIMED-BRIEF]` line is the leak-checked brief the auditor cleared, byte-for-byte — the SAME string you audited, and nothing after it.** Any persona preamble or attack-license line goes BEFORE the `[DEPRIMED-BRIEF]` line, never after the brief. The gate matches the post-tag brief against the receipt by **exact equality** (0.6.2): there is no longer any tolerance for trailing text — a trailing line, an appended "one more thing", or any edit after the audit changes the brief and draws NORECEIPT (correctly: it was never audited). That tolerance was removed because it was the hole a short steer slipped through. The gate passes silently only when a fresh receipt covers this exact brief; a tag with no receipt, or no tag at all, draws a reminder (or a hard block if `CLAIRE_GATE_STRICT=1` is set on this machine). If you *do* hit a NORECEIPT on a brief you believe you audited, set `CLAIRE_DEBUG=1` and re-dispatch: the gate will print the dispatched brief next to what was actually audited, so you can see the difference (a wrapper, an appended line, an edit) rather than assume it is a false alarm.
