...
already = any("record-audit-receipt.py" in h.get("command", "")
             for blk in post for h in blk.get("hooks", []))
if already:
    print("Already registered — receipt enforcement is wired in settings.json. Nothing to do.")
    sys.exit(0)
post.append({"matcher": "Agent|Task",
             "hooks": [{"type": "command", "command": cmd}]})
with open(settings_path, "w") as fh:
    json.dump(data, fh, indent=2)
print("Registered the receipt writer in", settings_path)
print("Receipt-backed de-priming enforcement will now fire on Agent/Task dispatches.")
